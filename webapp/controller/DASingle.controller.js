sap.ui.define(
  ...
  function (Controller, History, Filter, FilterOperator, Fragment) {
    ...
    return Controller.extend("commandearticle.controller.DASingle", {
      onInit: function () {
        const oRouter = sap.ui.core.UIComponent.getRouterFor(this); // Get the router instance
        oRouter
          .getRoute("RouteDASingle")
      ...
      _onItemMatched: function (oEvent) {
        ...
        aFilter.push(
          new Filter(
            ...
            JSON.parse(oSelectedItem).DemandeAchat
          )
        );
        ...
        const oDialog = new sap.m.Dialog({
          ...
          content: [
            new sap.m.Panel({
              ...
              headerText: `Résumé Articles`,
              content: that._onGetDialogContent(DAArticles),
            }),
            new sap.m.Panel({
              id: "Panel-container-1",
              headerText: `Résumé de la Commande`,
              content: [
                new sap.m.Text({
                  id: `Label-Vendor`,
                  text: "Fournisseur",
                }).addStyleClass('VendorLabel'),
                new sap.m.Input({
                ...
                new sap.m.Text({
                  id: `DateLivraison`,
                  text: "Date de Livraison Souhaité",
                }).addStyleClass('DateLabel'),
                new sap.m.Input({
          ...
          beginButton: new sap.m.Button({
            ...
            press: function () {
              ...
              const convertedDateSouhToTimestamp = new Date(
                sap.ui.getCore().byId('DateLivraisonSouhaitee').getValue().split('-')[0],
                sap.ui.getCore().byId('DateLivraisonSouhaitee').getValue().split('-')[1], 
                sap.ui.getCore().byId('DateLivraisonSouhaitee').getValue().split('-')[2]).getTime()
              // oBusyDialog.open();
              ...
              const order_header = {
                  ...
                  Datecreation: `\/Date(${new Date().getTime()})\/`,
                  Statut: "", // By default I use this, To confirm
